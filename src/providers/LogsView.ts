...
export class LogsView implements TreeDataProvider<LogItem> {
	...
	public static initialize(commands, context: ExtensionContext, dwConfig$$: Observable<Observable<Uri>>) {
	...
	public readonly onDidChangeTreeData: Event<LogItem | undefined> = this._onDidChangeTreeData.event;
	...
	public getTreeItem(element: LogItem): TreeItem {
		return element;
	}
	public cleanLog(logItem: LogItem) {
	...
	public openLog(logItem: LogItem) {
		const webdavClient = this.webdavClients.get(logItem.hostname);
		if (webdavClient) {
			window.withProgress({
						...
						return Observable.merge(...workspaceFolders$)
							...
							.flatMap((cartridges) => {
								...
								filedata = filedata.replace(/ (.*?)[:#](\d*)/ig, ($0, debuggerPath, $2) => {
									...
									const file = Uri.file(join(cartPath, ...debuggerSep));
								...
								return Observable.fromPromise(workspace.openTextDocument({ language: "dwlog", content: filedata })
									.then((document) => {
										...
										const newLines = filedata.substr(lastIndex).split("\n").length;
	...
	public async getChildren(element?: LogItem) {
