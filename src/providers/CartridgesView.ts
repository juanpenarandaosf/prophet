"use strict";
import { commands, Event, EventEmitter, ExtensionContext, RelativePattern, TreeDataProvider, TreeItem, TreeItemCollapsibleState, Uri, ViewColumn, window, workspace } from "vscode";

import { close, mkdir, open } from "fs";
import { basename, dirname, join } from "path";

import { CartridgeCreator, checkIfCartridge, getPathsCartridges } from "../lib/CartridgeHelper";
import { CartridgeTreeItem, DirectoryTreeItem, FileTreeItem, GenericTreeItem, WorkspaceTreeItem } from "../lib/CartridgeViewsItem";
...
export class CartridgesView implements TreeDataProvider<GenericTreeItem> {
	...
	public readonly onDidChangeTreeData: Event<GenericTreeItem | undefined> = this._onDidChangeTreeData.event;
	 ...
	 */
	public refresh(file?: string): void {
		if (file) {
	...
	public getTreeItem(element: GenericTreeItem): TreeItem {
		return element;
	}

	public async getChildren(element?: GenericTreeItem): Promise<GenericTreeItem[]> {
