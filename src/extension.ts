...
import { commands, debug, ExtensionContext, RelativePattern, Uri, window, workspace, WorkspaceConfiguration, WorkspaceFolder } from "vscode";
...
import { findFiles, getCartridgesFolder, getDWConfig } from "./lib/FileHelper";
...
function initFS(context: ExtensionContext) {
	...
	getDWConfig(fileWorkspaceFolders).then((options) => {
		...
		const sandboxFS = new SandboxFS(options);
				...
				.then((configData) => {
					if (workspace.workspaceFolders) {
						...
						return Promise.all(fileWorkspaceFolders.map(
							...
							const flatten = ([] as string[]).concat(...projects);
