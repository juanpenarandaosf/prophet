...
import { FileSystemWatcher, OutputChannel, Progress, ProgressLocation, RelativePattern, Uri, window, workspace } from "vscode";
import { getDirectories, stat } from "../lib/FileHelper";
import { DavOptions, default as WebDav } from "./WebDav";
...
function fileWatcher(config, cartRoot: string): Observable<["change" | "delete" | "create", string]> {
	return Observable.create((observer) => {
		...
		cartridges.then((cartridges) => {
			...
			const callback = (method: "change" | "delete" | "create") => ((uri: Uri) => {
...
const uploadCartridges = (
	...
	const cartridges: string[] = config.cartridge;
	let count = 0;
...
export function init(dwConfig: DavOptions, outputChannel: OutputChannel, config: { cleanOnStart: boolean, ignoreList: string[] }, ask: (sb: string[], listc: string[]) => Promise<string[]>) {
